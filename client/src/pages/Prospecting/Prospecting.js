...
const Prospecting = () => {
  ...
  const onSort = (columnId, order = "asc") => {
    setSortConfig({ columnId, order });
  };
  useEffect(() => {
    ...
    async (isRefresh = false, selectedPeriod = period, filteredIds = []) => {
      ...
      try {
        ...
        if (response.statusCode === 200 && response.success) {
          ...
          // const pmDataGrouping = await fetchProspectingActivityTypeGroupings(
  ...
  const fetchPaginatedData = useCallback(
    async (newPage, newRowsPerPage, newSearchTerm, newColumnId, newOrder) => {
        ...
        {error ? (
          ...
          <>
            <AccountDetail
              ...
              detailedActivationData={detailedActivationData}
            ...
            />
            {selectedActivation && (
              <SelectedAccount selectedActivation={selectedActivation} />
            )}
          </>
        )}
      </Box>
